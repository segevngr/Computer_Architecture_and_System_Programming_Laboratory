...
do_Str:                        		; do_Str function definition - functions are defined as labels
	...
	yourCode:			
        cmp byte [ecx], 9               ; Compare current char to Tab
        je REPLACE                      ; If Equal - Jumps to replace code
        cmp byte [ecx], 32              ; Compare current char to Space
        jne END                         ; If equal - continues to replace, else, skips replacement and jumps to the END of the loop
        REPLACE:                        
        mov byte [ecx], 95              ; Replaces space with '_'
        inc byte [an]                   ; Incrementing counter
        END:                            
