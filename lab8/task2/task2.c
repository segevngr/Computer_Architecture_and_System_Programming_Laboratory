...
//*****************Task 2*******************//
void printSymbols(elfFile *s) {
    Elf32_Ehdr *header = (Elf32_Ehdr *) s->map_start;
    Elf32_Shdr *sections = (Elf32_Shdr *) (s->map_start + s->header->e_shoff);
    Elf32_Shdr *sectionNamesTable = sections + header->e_shstrndx;
    char *sectionsNames = s->map_start + sectionNamesTable->sh_offset;
    for (int i = 0; i < header->e_shnum; i++)   // Iterates over header enteries
    {
        Elf32_Shdr *section = sections + i;
        if (section->sh_type == SHT_SYMTAB) // Enters when reaching symbol table
        {
            Elf32_Sym *allSymbols = s->map_start + section->sh_offset;
            int numOfSymbols = section->sh_size / section->sh_entsize;
            if(s->debug_mode == '1') {
                printf("Symbols table size: %d \n", section->sh_size);
                printf("Number of symbols: %d \n", numOfSymbols);
            }
            Elf32_Shdr *stringSection = sections + section->sh_link;
            char *symbolsNames = s->map_start + stringSection->sh_offset;
            printf("index | value | secInd | sectionName | symbolName \n");
            for (int i = 0; i < numOfSymbols; i++) {
                Elf32_Sym *symbol = allSymbols + i;
                char *symbolName = symbolsNames + symbol->st_name;
                int symbolIndex = symbol->st_shndx;
                char *sectionName;
                if(symbolIndex == SHN_UNDEF)
                    sectionName = "UND";
                else if(symbolIndex == SHN_ABS)
                    sectionName = "ABS";
                else if(symbolIndex == SHN_COMMON)
                    sectionName = "COM";
                else
                    sectionName = sectionsNames + sections[symbolIndex].sh_name;
                
                printf("[%2d] %08x %-8d %-20s %s\n", i, symbol->st_value, symbolIndex, sectionName, symbolName);
            }
        }
    }
}
